...
static DListNode* dlist_get_node(DList* thiz, size_t index, int fail_return_last)
{
	...
	if(!fail_return_last)				/*当fail_return_last取0的时判断在链表已经到tail的情况下，index是否在链表范围内。*/
	{
		iter = index > 0 ? NULL : iter;	//因为while循环结束的原因有3个,如果是前面2个条件而导致循环结束,要查看是否index已取完
	}
...
DListRet dlist_insert(DList* thiz, size_t index, void* data)
{
	...
	if(index < dlist_length(thiz))				//在链表范围内
	{
		node->next = cursor;
		cursor->prev = node;
		if(thiz->first == cursor)			//是头节点
		{
			thiz->first = node;
		}
	}
...
DListRet dlist_prepend(DList* thiz, void* data)			//一直添加first节点
{
	return dlist_insert(thiz, 0, data);
}

DListRet dlist_append(DList* thiz, void* data)			//在链表尾添加节点
{
